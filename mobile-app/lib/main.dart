...
class OrderService {
  ...
  Future<String> createOrder({
    ...
    final customerSnap = await db.collection('users').doc(uid).get();
    if (!customerSnap.exists) throw Exception('حساب العميل غير موجود');
    final customer = customerSnap.data()!;
    if (customer['role'] != 'customer' || customer['status'] != 'active') throw Exception('حساب العميل غير نشط');
    final customerState = '${customer['state'] ?? ''}'.trim();
    if (customerState.isEmpty) throw Exception('الولاية غير محددة في حسابك');
    ...
    final ref = await db.collection('orders').add({
      'customerId': uid, 'driverId': null, 'state': customerState,
  ...
  Future<void> updateStatus(String orderId, String status) async {
    ...
    await db.runTransaction((tx) async {
      ...
      if (status == 'delivering') patch['startedAt'] = FieldValue.serverTimestamp();
      if (status == 'awaiting_confirmation') patch['deliveredAt'] = FieldValue.serverTimestamp();
      tx.update(ref, patch);
    });
  }
        ...
        builder: (context, snapshot) {
          ...
          return Scaffold(appBar: AppBar(title: const Text('تفاصيل الطلب')), body: ListView(padding: const EdgeInsets.all(18), children: [
            ...
            Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              if (data['status'] == 'completed' && role == 'customer') ...[
...
).hasMatch(phone.trim())) throw Exception('رقم الهاتف يجب أن يكون 10 أرقام فقط');
    if (password.length < 6) throw Exception('كلمة المرور يجب أن تكون 6 أحرف على الأقل');
    if (!['customer', 'driver'].contains(role)) throw Exception('نوع الحساب غير صحيح');
    const states = ['الخرطوم','الجزيرة','القضارف','كسلا','البحر الأحمر','نهر النيل','الشمالية','النيل الأبيض','النيل الأزرق','سنار','شمال كردفان','جنوب كردفان','غرب كردفان','شمال دارفور','جنوب دارفور','غرب دارفور','وسط دارفور','شرق دارفور'];
    if (!states.contains(state.trim())) throw Exception('اختر ولاية صحيحة');
